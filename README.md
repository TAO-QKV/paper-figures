...
**Cross-domain showcase** — genomics, omics, clinical evidence, vector fields, and 3-D response surfaces in one reproducible figure ([`examples/showcase_gallery.py`](examples/showcase_gallery.py)):

![Showcase gallery](examples/gallery_showcase.png)

**Single-claim figure** — fitted response with a 95% CI band, the optimum annotated with its uncertainty, units on both axes, N stated, colorblind-safe and grayscale-legible. Generated by [`examples/gallery.py`](examples/gallery.py):
...
**Advanced high-impact figures** — Manhattan, streamplot, 3-D surface, and Sankey flow ([`examples/advanced_gallery.py`](examples/advanced_gallery.py)):

![Advanced gallery](examples/gallery_advanced.png)
...
**33 callable chart types**, each returning `(fig, ax)`:
...
| `examples/` | runnable galleries (`showcase_gallery.py`, `scientific_gallery.py`, `advanced_gallery.py`, `archetypes_gallery.py`, `paradigms_gallery.py`) + a complete TikZ hero (`hero_tikz/pipeline_hero.tex`) |
